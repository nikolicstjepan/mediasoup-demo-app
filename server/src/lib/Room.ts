...
import protoo, { AcceptFn, Peer, ProtooRequest, RejectFn, WebSocketTransport } from "protoo-server";
...
type BroadcasterType = {
    ...
    data: {
        ...
        rtpCapabilities: any;
        transports: Map<string | number, Transport>;
        producers: Map<string | number, Producer>;
        consumers: Map<string | number, Consumer>;
        dataProducers: Map<string | number, DataProducer>;
        dataConsumers: Map<string | number, DataConsumer>;
    };
};
...
class Room extends EventEmitter {
    ...
    _broadcasters: Map<string, BroadcasterType>;
     ...
     */
    static async create({ mediasoupWorker, roomId }: { mediasoupWorker: Worker; roomId: string }) {
    ...
    async handleProtooConnection({
        ...
        try {
            ...
            (peer.data.transports = new Map<string | number, Transport>()),
                (peer.data.producers = new Map<string | number, Producer>()),
                (peer.data.consumers = new Map<string | number, Consumer>()),
                (peer.data.dataProducers = new Map<string | number, DataProducer>()),
                (peer.data.dataConsumers = new Map<string | number, DataConsumer>()),
    ...
    async createBroadcaster({
        ...
        const broadcaster = {
            ...
            data: {
                ...
                rtpCapabilities,
                transports: new Map<string | number, Transport>(),
                producers: new Map<string | number, Producer>(),
                consumers: new Map<string | number, Consumer>(),
                dataProducers: new Map<string | number, DataProducer>(),
                dataConsumers: new Map<string | number, DataConsumer>(),
            },
        };
    ...
    async createBroadcasterTransport({
        ...
        switch (type) {
            case "webrtc": {
                ...
                const transport = await this._mediasoupRouter.createWebRtcTransport(options);
        ...
        switch (request.method) {
            ...
            case "applyNetworkThrottle": {
                ...
                try {
                    ...
                    reject(500, (error as Error).toString());
                }
     ...
     */
    _getJoinedPeers({ excludePeer }: { excludePeer?: Peer } = {}) {
    ...
    async _createConsumer({
        ...
        const transport = (Array.from(consumerPeer.data.transports.values()) as Transport[]).find(
    ...
    async _createDataConsumer({
        ...
        dataConsumerPeer: Peer;
        dataProducer: any;
        dataProducerPeer?: BroadcasterType | protoo.Peer; // This is null for the bot DataProducer.
    }) {
        ...
        // Must take the Transport the remote Peer is using for consuming.
        const transport = (
            Array.from(dataConsumerPeer.data.transports.values()) as Transport[]
